...
public class BlinkController : AnimController
{
	...
	protected bool _UpdateBlink( int blinkMTIndex, float time )
	{
		...
		if( nbt < blinkEaseInTime )
		{
			...
			morphCtrl.morphChannels[blinkMTIndex].weight = dw;
		}
		else if( nbt >= blinkEaseInTime && nbt <= beist )
		{
			// Sustain blink for a while
			
			morphCtrl.morphChannels[blinkMTIndex].weight = curBlinkMaxWeight;
		}
		else if( nbt > beist && nbt <= 1 )
		{
			// Blink in ease-out phase
			...
			morphCtrl.morphChannels[blinkMTIndex].weight = curBlinkDecayWeight + dw;
		}
		else
		{
			// Blink finished
			...
			morphCtrl.morphChannels[blinkMTIndex].weight = curBlinkDecayWeight;
			
			return true;
		}
		
		if( blinkMTIndex == lBlinkMTIndex )
			lBlinkWeight = morphCtrl.morphChannels[blinkMTIndex].weight;
		else if( blinkMTIndex == rBlinkMTIndex )
			rBlinkWeight = morphCtrl.morphChannels[blinkMTIndex].weight;
	...
	protected virtual void _DroopEyelids()
	{
		...
		if( StateId != (int)BlinkState.Blinking )
		{
			...
			curBlinkDecayTime -= Time.deltaTime;
		...
		morphCtrl.morphChannels[lBlinkMTIndex].weight += droop;
		morphCtrl.morphChannels[rBlinkMTIndex].weight += droop;
		
		lBlinkWeight = morphCtrl.morphChannels[lBlinkMTIndex].weight;
