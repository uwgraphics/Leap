...
public class DirectableJoint
{
	...
	public virtual Vector3 FaceDirection
	{
		get
		{
			return ( helper.position - bone.position ).normalized;
		}
		set
		{
			Quaternion rot = Quaternion.FromToRotation( ( helper.position - bone.position ).normalized, value );
			bone.rotation *= rot;
		}
	}
	...
	private Quaternion _ComputeCOBRotation()
	{
		...
		Vector3 vroll = ( bone.position - helper.position ).normalized;
