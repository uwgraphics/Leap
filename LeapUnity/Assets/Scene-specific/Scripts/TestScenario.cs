...
public class TestScenario : Scenario
{
	...
	protected virtual void _GenerateGazeTargets()
	{
		...
		foreach( GazeJoint eye in gctrl.eyes )
			ep += eye.bone.position;
		ep /= gctrl.eyes.Length;
	...
	{	
		...
		for( int gsi = 0; gsi < num_gs; ++gsi )
		{
			...
			else
			{
				...
				GazeController gctrl = agents[agentName].GetComponent<GazeController>();
				Quaternion currot = gctrl.LEye.bone.localRotation;
				gctrl.LEye.bone.localRotation =
					gctrl.LEye._ComputeTargetRotation( gsi > 0 ? gtTestPos1[ gazeShifts[gsi-1].targetId ] :
					                                  cameras["Main Camera"].transform.position );
				float y0 = gctrl.LEye.Yaw;
				float p0 = gctrl.LEye.Pitch;
				gctrl.LEye.bone.localRotation =
					gctrl.LEye._ComputeTargetRotation( gtTestPos1[gtid] );
				float y1 = gctrl.LEye.Yaw;
				float p1 = gctrl.LEye.Pitch;
				gctrl.LEye.bone.localRotation = currot;
				float y01 = y1 - y0;
				float p01 = p1 - p0;
