...
public class CatmullRomSpline
{
    ...
    public Vector3 GetPoint(float t)
    {
        ...
        float fcpi = t * (float)(controlPoints.Length - 1);
        int cpi = (int)Math.Floor(fcpi);
        if (cpi >= controlPoints.Length - 1)
    ...
    {
        if (tangents.Length != controlPoints.Length)
            RecomputeTangents();
    ...
    public Vector3 GetTangent(float t)
    {
        ...
        float fcpi = t * (float)(controlPoints.Length - 1);
        int cpi = (int)fcpi;
        if (cpi >= controlPoints.Length)
            cpi = controlPoints.Length - 1;
    ...
    public Vector3 GetTangent(int index, float t)
    {
        ...
        if (index >= controlPoints.Length - 1)
            return tangents[controlPoints.Length - 1];
